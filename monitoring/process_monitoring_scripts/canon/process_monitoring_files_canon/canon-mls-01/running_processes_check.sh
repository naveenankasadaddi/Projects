...
function check_ML_MDLSRV(){

MLMDLSRV_CHECK="4471"
MLMDLSRV_STATUS=$(ps aux | grep -v grep | grep HTTPProxyActor) #|grep $MLMDLSRV_CHECK)

var="up{}"

if [ "${#MLMDLSRV_STATUS}" -gt 0 ] ;  #Check for match
then
  var="$var""1"$'\n'         # If the process is up, then append 1.
  curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/mlmdlsrv/instance/"$host"  #Push the stats to PushGateway
else
  var="$var""0"$'\n'         # If the process is down, then append 0.
  curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/mlmdlsrv/instance/"$host"  #Push the stats to PushGateway
fi
