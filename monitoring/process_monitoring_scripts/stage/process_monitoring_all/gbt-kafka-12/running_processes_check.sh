...
function check_ZookeeperConnectionCounts(){
ConnectionCounts=$(sudo netstat -anp | grep :2182 | wc -l)

var="gb_dbconnectioncounts{}"
var="$var""${ConnectionCounts}"$'\n'
curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/zookeeperconcounts/instance/"$host"  #Push the stats to PushGateway
}

function check_KafkaConnectionCounts(){
ConnectionCounts=$(sudo netstat -anp | grep :9093 | wc -l)

var="gb_dbconnectioncounts{}"
var="$var""${ConnectionCounts}"$'\n'
curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/kafkaconcounts/instance/"$host"  #Push the stats to PushGateway
}
...
if [[ $# -lt 1 ]];then
  ...
  do
    if [ "$value" == "is" ];then
      ...
      check_alertengine
    elif [ "$value" == "KafkaConCounts" ];then
      check_ZookeeperConnectionCounts
      check_KafkaConnectionCounts
