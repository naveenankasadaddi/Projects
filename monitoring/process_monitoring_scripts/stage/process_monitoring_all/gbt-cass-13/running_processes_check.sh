...
if [ "$BC2R_STATUS" -eq 2 ] ;  #Check for match
then
  var="$var""1"$'\n'         # If the process is up, then append 1.
...
function check_CassandraConnectionCounts(){
ConnectionCounts=$(sudo netstat -anp | grep :9042 | wc -l)

var="gb_dbconnectioncounts{}"
var="$var""${ConnectionCounts}"$'\n'
curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/cassandraconcounts/instance/"$host"  #Push the stats to PushGateway
}
...
if [[ $# -lt 1 ]];then
  ...
  do
    if [ "$value" == "is" ];then
      ...
      check_vertica
    elif [ "$value" == "CassConCounts" ];then
      check_CassandraConnectionCounts
   fi
