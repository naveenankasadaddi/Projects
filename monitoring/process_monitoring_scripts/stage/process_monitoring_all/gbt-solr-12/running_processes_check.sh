...
function check_SolrConnectionCounts(){
ConnectionCounts=$(sudo netstat -anp | grep :8984 | wc -l)

var="gb_dbconnectioncounts{}"
var="$var""${ConnectionCounts}"$'\n'
curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/solrconcounts/instance/"$host"  #Push the stats to PushGateway
}
...
if [[ $# -lt 1 ]];then
  ...
  do
    if [ "$value" == "is" ];then
      ...
      check_alertengine
    elif [ "$value" == "SolrConCounts" ];then
      check_ZookeeperConnectionCounts
      check_SolrConnectionCounts 
   fi
