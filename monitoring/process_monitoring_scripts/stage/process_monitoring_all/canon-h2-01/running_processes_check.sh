...
BC2R_STATUS=$(ps aux | grep -v grep | grep $BC2R_CHECK|wc  -l)

var="up{}"

if [ "${BC2R_STATUS}" -ge 1 ] ;  #Check for match
then
  var="$var""1"$'\n'         # If the process is up, then append 1.
...
function check_ML_CANONAD(){

MLMGR_CHECK="9000"
MLMGR_STATUS=$(netstat -tnlp | grep $MLMGR_CHECK)

var="up{}"

if [ "${#MLMGR_STATUS}" -gt 0 ] ;  #Check for match
then
  var="$var""1"$'\n'         # If the process is up, then append 1.
  curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/mlcanonad/instance/"$host"  #Push the stats to PushGateway
else
  var="$var""0"$'\n'         # If the process is down, then append 0.
  curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/mlcanonad/instance/"$host"  #Push the stats to PushGateway
fi
...
then
  var="$var""1"$'\n'         # If the process is up, then append 1.
  curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/mlmgr/instance/"$host"  #Push the stats to PushGateway
else
  var="$var""0"$'\n'         # If the process is down, then append 0.
  curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/mlmgr/instance/"$host"  #Push the stats to PushGateway
fi
...
PORT_STATUS=$(netstat -tupln|grep -o -i $PORT_CHECK|wc -l)
...
if [[ $# -lt 1 ]];then
  ...
  do
    if [ "$value" == "is" ];then
      ...
      check_vertica
    elif [ "$value" == "ML-CANONAD" ];then
      check_ML_CANONAD
      
    elif [ "$value" == "ML-MGR" ];then
      check_ML_MGR
