...
TR_STATUS=$(ps aux | grep -v grep | grep $TR_CHECK |wc -l)

var="up{}"

if [ "$TR_STATUS" -eq 1 ] ;  #Check for match
then
  echo "Matched TR"
  var="$var""1"$'\n'         # If the process is up, then append 1.
...
FR_STATUS=$(ps aux | grep -v grep | grep -o -i $FR_CHECK|wc -l)

var="up{}"

if [ "$FR_STATUS" -eq 1 ] ;  #Check for match
then
  var="$var""1"$'\n'         # If the process is up, then append 1.
  echo Status OK curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/fr/instance/"$host"  #Push the stats to PushGateway
  ...
  var="$var""0"$'\n'         # If the process is down, then append 0.
  echo curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/fr/instance/"$host"  #Push the stats to PushGateway
  curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/fr/instance/"$host"  #Push the stats to PushGateway
fi
