...
SOLR_CHECK="start.jar"   #Process that is to be checked
SOLR_STATUS=$(ps aux | grep -v grep | grep $SOLR_CHECK)

var="up{}"  #This has to be re-initialised for every new process. Else, it will use the value which is set above.

if [ "${#SOLR_STATUS}" -gt 0 ] ;  #Check for match
then
  var="$var""1"$'\n'                         # If the process is up, then append 1.
...
ZOO_CHECK="org.apache.zookeeper.server.quorum.QuorumPeerMain"  #Process that is to be checked
...
CONSUL_CHECK="consul"
CONSUL_STATUS=$( ps aux | grep -v grep | grep $CONSUL_CHECK)

var="up{}"

if [ "${#CONSUL_STATUS}" -gt 0 ] ;  #Check for match
then
  var="$var""1"$'\n'         # If the process is up, then append 1.
  curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/consul/instance/"$host"  #Push the stats to PushGateway
else
  var="$var""0"$'\n'         # If the process is down, then append 0.
  curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/consul/instance/"$host"  #Push the stats to PushGateway
fi
...
then
  var="$var""1"$'\n'         # If the process is up, then append 1.
  curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/chronyd/instance/"$host"  #Push the stats to PushGateway
  echo "working"
else
  var="$var""0"$'\n'         # If the process is down, then append 0.
  curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/chronyd/instance/"$host"  #Push the stats to PushGateway
...
BC2R_STATUS=$(ps aux | grep -v grep | grep -o -i $BC2R_CHECK|wc -l)

var="up{}"

if [ "$BC2R_STATUS" -eq 3 ] ;  #Check for match
then
  var="$var""1"$'\n'         # If the process is up, then append 1.
...
F2V_CHECK="com.glassbeam.scalar.f2v.F2V"
F2V_STATUS=$(ps aux | grep -v grep | grep -o -i $F2V_CHECK|wc -l)

var="up{}"

if [ "$F2V_STATUS" -eq 1 ] ;  #Check for match
then
  var="$var""1"$'\n'         # If the process is up, then append 1.
  curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/f2v/instance/"$host"  #Push the stats to PushGateway
else
  var="$var""0"$'\n'         # If the process is down, then append 0.
  curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/f2v/instance/"$host"  #Push the stats to PushGateway
fi
...
function check_H2dbConnectionCounts(){
ConnectionCounts=$(sudo netstat -anp | grep :9092 | wc -l)

var="gb_dbconnectioncounts{}"
var="$var""${ConnectionCounts}"$'\n'
curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/h2dbconcounts/instance/"$host"  #Push the stats to PushGateway
}

function check_H2dbConnectionCountsRuState(){
ConnectionCounts=$(sudo netstat -anp | grep :9998 | wc -l)

var="gb_dbconnectioncounts{}"
var="$var""${ConnectionCounts}"$'\n'
curl -X POST -H  "Content-Type: text/plain" --data "$var"  http://"$pushgatewayhost":9091/metrics/job/h2rustatedbcounts/instance/"$host"  #Push the stats to PushGateway
}
...
if [[ $# -lt 1 ]];then
  ...
  echo "$0  cass,SparkMaster,SparkWorker,CassExporter,system" #C* with master & worker
  echo "$0  cass,SparkWorker,CassExporter,system" #C* with worker
  echo "$0  cass,CassExporter,system" # C* Without Worker
  echo "$0  solr,zoo_solr,system" #Solr with ZK
  echo "$0  solr,system" # Only Solr
  echo "$0  kafka,zoo_kafka,system" #Kafka, ZK
  echo "$0  FR,BR,TR,system" #FR, BR, TR
  echo "$0  C2S,system" #Only C2S
  echo "$0  LcpBatch,system" #Only LcpBatch
  echo "$0  H2,RM,SM,system" #H2, RM, SM
  echo "$0  H2,RM,SM,LcpBatch,system" #H2, RM, SM, LCP
  echo "$0  UAD-AUG,LCPAE,modelserver,system" #UAD, AE, ModelServer
  echo "$0  BC2R,system" #Only BC2R
  echo "$0  LCP-Rules,system" #Only LcpRules
  echo "$0  Vertica,system" #Only Vertica
  echo "$0  LCPAE,is,kafka,cass,zoo_kafka,SM,LCP-Rules,system"  # AE, IS, Kafka, C*, ZK, SM, LcpRules 
  echo "$0  ILB,SeenID,system"  # ILB,SeenID
  echo "$0  LCPAE,MLAE,system"  # LCP-AlertEngine,ML-AlertEngine,SSH,NTPD
  sleep 10
  exit
else
  ...
  do
    if [ "$value" == "is" ];then
    ...
    elif [ "$value" == "CONSUL" ];then
      check_consul

    elif [ "$value" == "chronyd" ];then
      check_chronyd      
      ...
      check_alertengine
    elif [ "$value" == "THREADS" ];then
      check_threadscount  
    elif [ "$value" == "H2ConCounts" ];then
      check_H2dbConnectionCounts
      check_H2dbConnectionCountsRuState 
    fi
